...
public void paint(Graphics g){//resive graphic object to draw the ball and everything
    //adding background
    Image image ;
     /*after downloading the gif from github please make sure that the gif downloaded 
    in the same folder as the code and here it should
    have the same name*/
    ...
    BallDir_y = 0; 
       ...
       g.setFont(new Font("serif",Font.BOLD,30));
       g.drawString("Press Enter To Go To The Next Level!",250,520);
    }
    ...
    if(ke.getKeyCode() == KeyEvent.VK_ENTER){ 
        ...
        if(!play && start != 0){
           ...
           //set everything again
            ...
            else{//if the player wins the last level, the game will restart
                rows = 2;
                cols = 11;
                TotalBricks = rows*cols;
                Brick = new Bricks(rows,cols);
                //change directions to it initial
                BallDir_x =-4;
                BallDir_y =-4;}
